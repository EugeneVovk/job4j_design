 ...
 * сериализованных данных в объекты,
 ...
 * В Java существует стандартный механизм сериализации в бинарный формат –
 * Java serialization, из текстовых форматов наиболее популярны
 * JSON, XML, YAML, BSON (binary JSON).
 * Java serialization.
 * Для стандартной сериализации объекта необходимо в классе имплементировать
 * интерфейс Serializable, этот интерфейс является маркерным,
 * т.е. нет необходимости реализовывать его методы, он сообщает JVM,
 ...
 * в реальной разработке лучше использовать штатный механизм
 * Java генерации serialVersionUID или разработать свой.
 ...
 * С помощью интерфейса Externalizable можно реализовать собственный алгоритм
 * сериализации/десериализации, для этого нужно переопределить два обязательных метода
