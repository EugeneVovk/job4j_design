...
select p.name Сыр from product p join type t on type_id =t.id where t.name = 'СЫР';
--2. Написать запрос получения всех продуктов, у кого в имени есть слово "мороженое"
select name Мороженое from product where name like '%мороженое%';
--3. Написать запрос, который выводит все продукты, срок годности которых уже истек
select name Продукт, expired_date "Срок годности" from product where expired_date < current_date;
...
select t.name Тип, count(p.name) "Количество продуктов" from product p join type t on type_id=t.id group by 1;
--6. Написать запрос получение всех продуктов с типом "СЫР" и "МОЛОКО"
select p.name from product p join type t on type_id=t.id where t.name = 'СЫР' or t.name = 'МОЛОКО'  order by t.name;
...
select t.name, count(p.name) from product p join type t on type_id = t.id group by t.name having count(p.name) < 10;
--8. Вывести все продукты и их тип.
select p.name Продукты, t.name Тип from product p join type t on type_id=t.id order by 2;